...
const ProjectCard = ({
  ...
  return (
    <motion.div
      ...
      className={`relative z-10 h-[700px] w-full items-stretch justify-center overflow-hidden rounded-3xl bg-center py-0 sm:h-[850px] sm:w-[100%] md:h-[800px] md:w-[100%] lg:h-[650px]`}
      initial="initial"
      animate="animate"
    >
      <Image
        ...
        style={{ bottom: id === 0 ? '8%' : (id === 3 ? '151px' : '94px') as any }}
      />
      <div
      ...
      <div
        ...
        style={id === 3 ? { transform: 'translateY(-133px)' } : {}}
      >
        <AnimatedTitle
          text={name}
          className={
      ...
      {id !== 2 && (
        <div className="absolute bottom-8 left-0 right-0 w-full px-4">
          <div className="flex flex-nowrap gap-4 bg-[#212531]/80 p-4 rounded-lg backdrop-blur-sm overflow-x-auto justify-center mx-auto w-[90%]">
            {technologies.map((tech, id) => (
              <AnimatedTitle
                ...
                key={id}
                className={
                  "text-[12px] font-bold uppercase text-white md:text-[14px] lg:text-[16px] whitespace-nowrap flex-shrink-0"
                }
              />
            ))}
          </div>
